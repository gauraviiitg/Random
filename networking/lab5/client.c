...
void send_acknowledgment(int sequence_num) {
    printf("Sending acknowledgment for Frame %d\n", sequence_num);
    char buffer[10];
    sprintf(buffer, "%d", sequence_num);
        ...
        if (bytes_received > 0) {
           ...
           // printf("Content of Frame %d: %s\n", received_frame.sequence_num, received_frame.data);
            ...
            if (received_frame.sequence_num == 6 && !dropped_6) {
                ...
                send(client_socket, "DROP", 4, 0); 
                dropped_8 = 1;
            } else if (received_frame.sequence_num == expected_sequence) {
                fprintf(file, "%s", received_frame.data); 
                send_acknowledgment(expected_sequence);
