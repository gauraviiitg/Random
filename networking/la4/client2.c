...
int main() {
    ...
    while ((bytes_received = recv(client_socket, buffer, FRAME_SIZE, 0)) > 0) {
        fwrite(buffer, 1, bytes_received, file);
        printf("Received frame.\n");
        frame_count++;

        if (frame_count == 6) {
            printf("Simulating lost ACK for frame 6.\n");
            Sleep(6000);
            continue;
        }

        if (frame_count == 7) {
            printf("Simulating lost ACK for frame 7.\n");
            Sleep(6000);
            continue;
        }

        if (frame_count == 8) {
            printf("Simulating lost ACK for frame 8.\n");
            Sleep(6000);
            continue;
        }
